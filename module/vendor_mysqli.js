...
mysqliq['get_saved_leads']='SELECT * from vendorproduct INNER JOIN product on vendorproduct.Pid=product.id WHERE vendorproduct.Uid=? AND (vendorproduct.product_status=? OR vendorproduct.product_status=? OR vendorproduct.product_status=?)';
...
mysqliq['vendor_contribution']='SELECT count(*) AS count from product  WHERE status=? AND accepted_vendor=? AND type=?';
...
mysqliq['vendor_month_sale']="SELECT  VP.product_status,count(*) as count,P.type from vendorproduct VP INNER JOIN product P on P.id=VP.Pid  WHERE VP.Uid=? AND (VP.product_status!=? AND VP.product_status!=? && P.type=?) AND VP.updated_at BETWEEN ? and ? GROUP BY product_status"
...
mysqliq['most_sold_product']='SELECT DISTINCT(product.type) from vendorproduct INNER JOIN product on vendorproduct.Pid=product.id WHERE vendorproduct.Uid=? '
